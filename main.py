...
async def update_address(address_id: int, address: AddressUpdate, db: Session = Depends(get_db)):
    ...
    db_address = db.query(Address).filter(Address.id == address_id).first() # Get the address with Given ID
    if db_address is None:
        # Raise error if address with Given ID not found
        logger.error('Address with id {} not found.'.format(address_id))
        raise HTTPException(status_code=404, detail="Address not found")
...
async def delete_address(address_id: int, db: Session = Depends(get_db)):
    ...
    db_address = db.query(Address).filter(Address.id == address_id).first() # Fetch Address associated with Given ID
    if db_address is None:
